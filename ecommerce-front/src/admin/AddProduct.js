...
const AddProduct  = ()=>{
       ...
       .then(data=>{
         if(data.error){
           setValues({...values, error: data.error})
            ...
            setValues({...values,name:'', description: '',photo: '',price: '',quantity: '',loading: false, createProduct: data.name})
         }
       });
    };
    ...
    const newPostForm = ()=> (
       <form className="mb-3" onSubmit={clickSubmit}>
        ...
        <div className="form-group">
          ...
          <input onChange = {handleChange('photo')} className= "form-control" type="file" name="photo" accept="image/*" />
          </label> 
        </div> 
    ...
    return ( 
        <Layout title="Add a new Product" description={`Good Day ${user.name}, ready to add a new category`} >
        <div className="row">
